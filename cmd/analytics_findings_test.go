...
func TestAnalyticsFindingsHumanAndRawJSONWithoutOrgQuery(t *testing.T) {
	...
	out, _, err := executeCommand("analytics", "findings")
	assert.NoError(t, err)
	assert.Contains(t, out, "Reduce duplicates")

	out, _, err = executeCommand("--json", "analytics", "findings")
	assert.NoError(t, err)
...
func TestMagnoliaFindingsQuiet(t *testing.T) {
	...
	out, stderr, err := executeCommand("analytics", "findings", "--quiet")
	assert.NoError(t, err)
	assert.Empty(t, out)
...
func TestMagnoliaFindingsAPIError(t *testing.T) {
	...
	_, stderr, err := executeCommand("analytics", "findings")
	require.Error(t, err)
...
func TestLegacyMagnoliaFindingsRequiresAndSendsOrgID(t *testing.T) {
	setupAPIServer(t, func(w http.ResponseWriter, r *http.Request) {
		assert.Equal(t, "org_legacy", r.URL.Query().Get("orgId"))
		_, _ = w.Write([]byte(`{"run":{},"findings":[],"groups":[]}`))
	})
	_, _, err := executeCommand("magnolia", "findings")
	require.Error(t, err)
	_, _, err = executeCommand("magnolia", "findings", "--org-id=")
	assert.EqualError(t, err, "--org-id is required")
	_, _, err = executeCommand("magnolia", "findings", "--org-id", "org_legacy", "--quiet")
	assert.NoError(t, err)
}
